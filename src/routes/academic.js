    ...
    try {
      ...
      if (!validateACIDs(slotRep))
        return res
          ...
          .json({
            msg:
              "Please enter slotRep as an array of valid academic staff ids.",
          });
      ...
      for (i = 0; i < reqs.length; i++) {
        await Notification.create(
          { message: reqs[i], date: Date.now(), read: false },
          async (err, notif) => {
            if (err) throw err;
  ...
  async (req, res) => {
    try {
      ...
      if (taken)
        return res
          .status(500)
          .json({ msg: "Anoter staff member was assigned to that slot." });
      ...
      sender.notifications.forEach((notif) => {
        if (
          notif.message.senderID === user.objectID &&
      ...
      sender.schedule.forEach((slot) => {
        if (slot.weekday === dayOff)
          return res
            ...
            .json({
              msg:
                "There is/are slot(s) assigned during the requested day off.",
            });
      });
...
router.get("/notifications", authenticateAndAuthoriseAC, async (req, res) => {
  try {
    ...
    sender.notifications.forEach((n) => {
      if (n.message.senderID === user.objectID) subNotifs.push(n);
    });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      sender.notifications.forEach((n) => {
        if (
          n.message.senderID === user.objectID &&
