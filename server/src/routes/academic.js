...
const upload = multer({ storage: storage, fileFilter: isZipFile }).single(
  "document"
);
...
router.get("/schedule", authenticateAndAuthoriseAC, async (req, res) => {
  try {
    ...
    let schedule = await Staff.findOne({ _id: user.objectID }).populate({
      path: "schedule.course schedule.location",
      //    populate: {
  ...
  async (req, res) => {
    try {
      ...
      if (!replacementDate)
        return res
          .status(400)
          .json({ msg: "Please enter a date for replacement." });

      if (!slotRep)
        return res.status(400).json({
          msg:
            "Please enter an array of staff IDs for replacement (in the same order of the slots to be covered).",
        });
      ...
      if (!repDate)
        return res.status(400).json({
          msg:
            "Please enter replacementDate as a valid date string (yyyy-mm-dd).",
        });

      //check if the array entered has academic staffIDs in the right format
      if (!validateACIDs(slotRep))
        return res.status(400).json({
          msg: "Please enter slotRep as an array of valid academic staff ids.",
        });
      ...
      for (i = 0; i < schedule.length; i++) {
        if (schedule[i].weekday === weekday) slt.push(schedule[i]);
      }
      ...
      if (fourth) slots.push(fourth);
      if (fifth) slots.push(fifth);
      ...
      if (slots.length !== slotRep.length)
        return res.status(400).json({
          msg:
            "Please enter the amount of staff ids equal to the number of non free slots on the replacement day.",
        });
      ...
      for (i = 0; i < slots.length; i++) {
        ...
        if (!staff)
          return res
            .status(404)
            .json({ msg: "There is no staff with the id: " + slotRep[i] });
        ...
        if (!staff.courseIDs.includes(slots[i].course))
          return res.status(400).json({
            msg:
              ...
              slotRep[i] +
              " does not teach " +
              (await Course.findById(slots[i].course)).courseCode,
          });
      }
      ...
      for (i = 0; i < slots.length; i++) {
        await Replacement.create(
          { replacementDay: repDate, replacementSlot: slots[i] },
          async (err, rep) => {
            if (err) throw err;
            ...
            await Request.create(
              ...
              async (err, rq) => {
                ...
                await Notification.create(
                  { message: rq, date: Date.now(), read: false },
                  async (err, notif) => {
                    if (err) throw err;
  ...
  async (req, res) => {
    try {
      ...
      if (!slotID)
        return res
          .status(400)
          .json({ msg: "Please enter an id of the slot requested." });
      ...
      if (!slot)
        return res
          .status(404)
          .json({ msg: "There is no slot with the id entered." });
      ...
      if (!sender.courseIDs.includes(slot.course))
        return res.status(400).json({
          msg:
            "The course of the slot requested is not within the courses assigned to the user.",
        });
      ...
      if (taken)
        return res
          .status(500)
          .json({ msg: "Another staff member was assigned to that slot." });
      ...
      sender.schedule.forEach((schSlot) => {
        if (schSlot.weekday === slot.weekday && schSlot.number === slot.number)
          return res.status(400).json({
            msg:
              "There is already a slot assigned during the requested slot's time.",
          });
      });
      ...
      sender.notifications.forEach((notif) => {
        if (
          notif.message.senderID.equals(user.objectID) &&
          notif.message.linkingSlot &&
          notif.message.status !== "Rejected" &&
          notif.message.linkingSlot.slot.weekday === slot.weekday &&
          notif.message.linkingSlot.slot.number === slot.number
        )
          return res.status(400).json({
            msg:
              "There is a slot linking request sent for a slot during the requested slot's time.",
          });
      });

      //send the request

      const { coordinatorID } = await Course.findById(slot.course, {
        _id: 0,
        coordinatorID: 1,
      });
  ...
  async (req, res) => {
    try {
      ...
      if (!dayOff)
        return res
          .status(400)
          .json({ msg: "Please enter a weekday for the new day off." });
      ...
      if (!WEEKDAYS.includes(dayOff))
        return res.status(400).json({
          msg:
            "Please enter new day off as one of the following formats: " +
            WEEKDAYS,
        });
      ...
      if (dayOff === sender.dayOff)
        return res.status(500).json({
          msg:
            "New day off entered is the same as the what is currently assigned.",
        });
      ...
      sender.schedule.forEach((slot) => {
        if (slot.weekday === dayOff)
          return res.status(400).json({
            msg: "There is/are slot(s) assigned during the requested day off.",
          });
      });

      //LS req associated with requested day off => if pending, delete

      const { hodID } = await Department.findById(sender.departmentID, {
        _id: 0,
        hodID: 1,
      });
      ...
      sender.notifications.forEach(async (notif) => {
        if (
          notif.message.linkingSlot &&
          notif.message.linkingSlot.slot.weekday === dayOff &&
          notif.message.status === "Pending"
        ) {
          await LinkingSlot.findByIdAndDelete(
            notif.message.linkingSlot._id,
            async (err, ls) => {
              if (err) throw err;
  ...
  async (req, res) => {
    try {
      ...
      if (!startLeaveDate)
        return res
          .status(400)
          .json({ msg: "Please enter start date for the accidental leave." });
      if (!endLeaveDate)
        return res
          .status(400)
          .json({ msg: "Please enter the end date for the accidental leave." });
      ...
      if (!startDate)
        return res.status(400).json({
          msg:
            "Please enter the accidental start leave date as a valid date string (yyyy-mm-dd).",
        });
      ...
      if (!endDate)
        return res.status(400).json({
          msg:
            "Please enter the accidental end leave date as a valid date string (yyyy-mm-dd).",
        });
      ...
      if (endDate.getTime() - startDate.getTime() < 0)
        return res
          .status(400)
          .json({ msg: "End date should be after the start date." });
      ...
      if (startDate < Date.now())
        return res
          .status(400)
          .json({ msg: "Annual leave start date already passed." });
      ...
      if (floor(sender.leaveBalance) < days)
        return res.status(500).json({
          msg:
            "There is not enough annual leave balance for the requested accidental leave.",
        });
      ...
      switch (sender.dayOff) {
        ...
        default:
          throw Error(
            "Day Off input in the database does not follow valid format. "
          );
      }
      ...
      return res.status(200).json({
        msg:
          "Annual leave request sent. Please send replacement requests for the following working days:" +
          "\n" +
          workingDays,
      });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (sender.accidentDays < days)
        return res.status(500).json({
          msg:
            "There is not enough accidental leave days for the requested accidental leave.",
        });
  ...
  async (req, res) => {
    try {
      ...
      if (!startLeaveDate)
        return res
          .status(400)
          .json({ msg: "Please enter start date for the sick leave." });
      if (!endLeaveDate)
        return res
          .status(400)
          .json({ msg: "Please enter the end date for the sick leave." });
      ...
      if (!startDate)
        return res.status(400).json({
          msg:
            "Please enter the sick start leave date as a valid date string (yyyy-mm-dd).",
        });
      ...
      if (!endDate)
        return res.status(400).json({
          msg:
            "Please enter the sick end leave date as a valid date string (yyyy-mm-dd).",
        });
      ...
      if ((Date.now() - startDate.getTime()) / (60 * 60 * 24 * 1000) > 3)
        return res
          .status(400)
          .json({ msg: "Sick leave start date is more than 3 days ago." });
      ...
      upload(req, res, (err) => {
        ...
        if (!req.file)
          return res
            .status(400)
            .json({ msg: "Please upload a zip file of documents." });
  ...
  async (req, res) => {
    try {
      ...
      if (!startLeaveDate)
        return res
          .status(400)
          .json({ msg: "Please enter start date for the maternity leave." });
      if (!endLeaveDate)
        return res
          .status(400)
          .json({ msg: "Please enter the end date for the maternity leave." });
      ...
      if (!startDate)
        return res.status(400).json({
          msg:
            "Please enter the maternity start leave date as a valid date string (yyyy-mm-dd).",
        });
      ...
      if (!endDate)
        return res.status(400).json({
          msg:
            "Please enter the maternity end leave date as a valid date string (yyyy-mm-dd).",
        });
      ...
      if (sender.gender === "Male")
        return res
          .status(400)
          .json({ msg: "Only female staff can send maternity leaves." });
      ...
      if (
        ...
        return res.status(400).json({
          msg:
            "Duration of maternity leave is more than three months (90 days).",
        });
      ...
      if (startDate < Date.now())
        return res
          .status(400)
          .json({ msg: "Maternity leave start date already passed." });
...
router.post(
  "/compensation-leave-request/send",
  authenticateAndAuthoriseAC,
  async (req, res) => {
    try {
      //gets the payload of the token
      ...
      if (!compensationLeaveDate)
        return res
          .status(400)
          .json({ msg: "Please enter the date for the leave." });
      if (!compensationDate)
        return res
          .status(400)
          .json({ msg: "Please enter the date for the compensation." });
      if (!reason)
        return res
          .status(400)
          .json({ msg: "Please enter the reason for the compensation leave." });
      ...
      if (!leaveDate)
        return res.status(400).json({
          msg:
            "Please enter the compensation leave date as a valid date string (yyyy-mm-dd).",
        });
      const compDate = validateDate(compensationDate);
      if (!compDate)
        return res.status(400).json({
          msg:
            "Please enter the compensation date as a valid date string (yyyy-mm-dd).",
        });
      ...
      if (leaveDate < Date.now())
        return res
          .status(400)
          .json({ msg: "Leave date input already passed." });
      ...
      if (weekday === "Friday" || weekday === sender.dayOff)
        return res
          .status(400)
          .json({ msg: "Date requested for the leave is already a day off." });
      ...
      if (compDay !== "Friday" && compDay != sender.dayOff)
        return res
          .status(400)
          .json({ msg: "Compensation date is not a day off." });
      ...
      if (leaveDate.getDate() >= 11) {
        ...
        if (month === 11) endDate = new Date(year + 1, 0, 10);
        else endDate = new Date(year, month + 1, 10);
      } else {
        const year = leaveDate.getFullYear();
        const month = leaveDate.getMonth();
        if (month === 0) startDate = new Date(year - 1, 11, 11);
        else startDate = new Date(year, month - 1, 11);
        endDate = new Date(year, month, 10);
      }

      if (compDate > endDate || compDate < startDate)
        return res.status(400).json({
          msg: "Compensation date is not within the same month as the leave.",
        });
...
router.get("/notifications", authenticateAndAuthoriseAC, async (req, res) => {
  try {
    ...
    sender.notifications.forEach((n) => {
      if (n.message.senderID.equals(user.objectID)) subNotifs.push(n);
    });
    ...
    if (subNotifs.length > 0) {
      ...
      sender.notifications.forEach((n) => {
        if (subNotifsIds.includes(n._id)) n.read = true;
      });
    ...
    //otherwise return a message indicating there is no notification to show
      ...
      return res
        .status(200)
        .json({ msg: "There are currently no notifications." });
  } catch (error) {
    res.status(500).json({ msg: error.message });
  }
});
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      sender.notifications.forEach((n) => {
        if (
          n.message.senderID.equals(user.objectID) &&
          n.message.status === "Accepted"
        )
          accNotifs.push(n);
      });
      ...
      if (accNotifs.length > 0) {
        ...
        sender.notifications.forEach((n) => {
          if (accNotifIds.includes(n._id)) n.read = true;
        });
        await sender.save();
      ...
      //otherwise return a message indicating there is no notification to show
        ...
        return res
          .status(200)
          .json({ msg: "There are currently no accepted notifications." });
    } catch (error) {
      res.status(500).json({ msg: error.message });
    }
  }
  ...
  async (req, res) => {
    try {
      ...
      if (rejNotifs.length > 0) {
        ...
        sender.notifications.forEach((n) => {
          if (rejNotifsIds.includes(n._id)) n.read = true;
        });
        await sender.save();
      ...
      //otherwise return a message indicating there is no notification to show
        ...
        return res
          .status(200)
          .json({ msg: "There are currently no rejected notifications." });
    } catch (error) {
      res.status(500).json({ msg: error.message });
    }
  }
...
router.get("/requests", authenticateAndAuthoriseAC, async (req, res) => {
  try {
    ...
    if (result.length > 0) return res.status(200).json({ requests: result });
    //otherwise return a message indicating ther is no notification to show
    else
      return res
        .status(200)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      //if there are requests return them to the user
      if (reqs.length > 0) return res.status(200).json({ requests: reqs });
      //otherwise return a message indicating ther is no notification to show
      else
        return res
          .status(200)
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      if (!reqID)
        return res.status(400).json({
          msg: "Please enter the id of the request you want to cancel.",
        });
      ...
      if (!request.senderID.equals(user.objectID))
        return res
          .status(400)
          .json({ msg: "Request is sent by a different staff member." });
      ...
      if (request.status !== "Pending")
        return res
          .status(400)
          .json({ msg: "Request already received a response." });
      ...
      if (request.leave) {
        if (request.leave.startDate < Date.now())
          return res
            .status(400)
            .json({ msg: "Start date of leave already passed." });
      } else if (request.replacement) {
        if (request.replacement.replacementDate < Date.now())
          return res
            .status(400)
            .json({ msg: "Replacement date already passed." });
      }
      ...
      //RP
        ...
        //A) delete the RP
        await Replacement.findByIdAndDelete(
          request.replacementDate._id,
          async (err, rp) => {
            if (err) throw err;
